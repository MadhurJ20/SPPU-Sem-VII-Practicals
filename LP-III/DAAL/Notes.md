...
- **Answer:**
  ...
  - _Greedy choice property_: A global optimum can be arrived at by
    selecting the local optimum.
  - _Optimal substructure_: A problem has an optimal substructure if
...
O((n - m + 1) \* m), where _n_ is the length of the text and _m_ is the
